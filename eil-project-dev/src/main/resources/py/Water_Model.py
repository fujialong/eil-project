...
#河流完全混合模式
def River_Completely_Mixed(Ch,Cp,Qp,Qh):
    c = (Cp*Qp+Ch*Qh)/(Qp+Qh)
    return c

#二维稳态混合模式（岸边排放）
def Two_Steady_Shore(x,y,Ch,Cp,Qp,H,My,u,B):
    part1 = e**((u*y**2)/(4*My*x))
    part2 = e**(-u*((2*B-y)**2)/(4*My*x))
    c = Ch+Cp*Qp/(H*sqrt(pi*My*x*u))*(part1+part2)
    return c

#二维稳态混合模型（非岸边排放）
def Two_Steady_Nonshore(x,y,Ch,Cp,Qp,H,My,u,B,a):
    part1 = e**(-u*y**2/(4*My*x))
    part2 = e**(-u*(2*a+y)**2/(4*My*x))
    part3 = e**(-u*(2*B-2*a-y)**2/(4*My*x))
    c = Ch+Cp*Qp/(2*H*sqrt(pi*My*x*u))*(part1+part2+part3)
    return c

#二维稳态混合衰减模式（岸边排放）
def Two_Steady_Attenuation_Shore(x,y,Ch,Cp,Qp,H,My,u,B,K1):
    part1 = e**(-K1*x/(86400*u))
    part2 = e**(-u*y**2/(4*My*x))
    part3 = e**(-u*(2*B-y)**2/(4*My*x))
    c = part1*(Ch+Cp*Qp/(H*sqrt(pi*My*x*u))*(part2+part3))
    return c

#二维稳态混合衰减模式（非岸边排放）
def Two_Steady_Attenuation_Nonshore(x,y,Ch,Cp,Qp,H,My,u,B,a,K1):
    part1 = e ** (-K1 * x / (86400 * u))
    part2 = e ** (-u * y ** 2 / (4 * My * x))
    part3 = e ** (-u*(2*a+y)**2/(4*My*x))
    part4 = e ** (-u*(2*B-2*a-y)**2/(4*My*x))
    c = part1*(Ch+Cp*Qp/(2*H*sqrt(pi*My*x*u))*(part2+part3+part4))
    return c

#托马斯模式
def Thomas_Mode(x,Ch,Cp,Qp,Qh,Dp,Dh,u,K1,K2,K3):
    c0 = (Cp*Qp + Ch*Qh)/(Qp+Qh)
    D0 = (Dp*Qp + Dh*Qh) /(Qp+Qh)
    c = c0*e**(-(K1+K3)*x/(86400*u))
    part1 = e**(-(K1+K3)*x/(86400*u))
    part2 = e**(-K2*x/(86400*u))
    D = K1*c0/(K2-(K1+K3))*(part1-part2)+D0*part2
    xc = u/(K2-(K1+K3))*log(K2/(K1+K3) + K2*(K1+K3-K2)*D0/(K1*(K1+K3)*c0))
    return c,D,xc

#欧康纳河口衰减模式（上溯时&下泄时）
def OConnor_Attenuation(x,Ch,Cp,Qp,Qh,Ml,u,K1):
    M = sqrt(1+4*K1*Ml/u**2)
    if x<=0:
        c = Cp*Qp/((Qh+Qp)*M)*e**(u*x/(2*Ml)*(1+M))+Ch
    if x>0:
        c = Cp*Qp/((Qh+Qp)*M)*e**(u*x/(2*Ml)*(1-M))+Ch
    return c

#湖泊稳态二维环流混合模式（岸边排放）
def Lake_Two_Steady_Shore(x,y,Ch,Cp,Qp,H,My,u):
    part1 = e**(-x*y**2/(4*My*x))
    c = Ch+Cp*Qp/(H*sqrt(pi*My*x*u))*part1
    return c

#湖泊稳态二维环流混合模式（非岸边排放）
def Lake_Two_Steady_Nonshore(x,y,Ch,Cp,Qp,H,My,u,a):
    part1 = e**(-x*y**2/(4*My*x))
    part2 = e**(-u*(2*a+y)**2/(4*My*x))
    c = Ch+Cp*Qp/(2*H*sqrt(pi*My*x*u))*(part1+part2)
    return c

#湖泊稳态二维混合衰减混合模式（岸边排放）
def Lake_Two_Steady_Attenuation_Shore(x,y,Ch,Cp,Qp,H,My,u,K1):
    part1 = e**(-u*y**2/(4*My*x))
...
def Lake_Two_Steady_Attenuation_Nonshore(x,y,Ch,Cp,Qp,H,My,u,a,K1):
    part1 = e**(-x*y**2/(4*My*x))
    part2 = e**(-u*(2*a+y)**2/(4*My*x))
